...
const wallpapers = fs.readdirSync(fullDir)
  ...
  .map(file => {
    ...
    return {
      ...
      thumbnail: path.join(thumbDir, file.replace(/\.(\w+)$/, '-thumb.$1'))
    };
  });

fs.writeFileSync('src/data/wallpapers.json', JSON.stringify(wallpapers, null, 2));
